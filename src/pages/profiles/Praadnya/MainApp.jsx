...
const App = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await axios.get('http://localhost:8080/intern/Praadnya H');
  ...
  const pages = [
    ...
    <EducationPage education={portfolioData?.education} />,
            ...
            {pages.map((_, index) => (
              <motion.div
                ...
                className={`w-2 h-2 rounded-full cursor-pointer ${
                  currentPage === index ? 'bg-[#5c4934]' : 'bg-[#d4b595]'
                }`}
